...
for i in range(len(puzzle)):
   puzzle[i] = re.split("[\s-]", puzzle[i])
   puzzle[i][2] = puzzle[i][2][:-1]
   if (puzzle[i][3].count(puzzle[i][2]) >= int(puzzle[i][0]) and
       puzzle[i][3].count(puzzle[i][2]) <= int(puzzle[i][1])):
       total_valid += 1
...
#     puzzle[i][2] = puzzle[i][2][:-1]
#
#     pos1 = int(puzzle[i][0]) - 1
#     pos2 = int(puzzle[i][1]) - 1
#     character = puzzle[i][2]
#     password = puzzle[i][3]
#
